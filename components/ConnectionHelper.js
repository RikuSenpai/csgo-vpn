...
module.exports = class ConnectionHelper {
	static HandleNetMessage(buffer, country) {
		// Get eMsg
		...
		return this.HandleMessage(header, buf.slice(), country);
	}

	static async HandleMessage(header, bodyBuf, country) {
		// Decode
		let body = bodyBuf;
		...
		if (header.msg === EMsg.Multi) {
			mod = await Modifier.multi(this, header, body, country);
		} else {
			mod = await Modifier.run(header, body, country);
		}
